...
describes what users get. Pick a format with `-o`, or let acl choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
acl <command> -o table   # aligned columns for reading
acl <command> -o jsonl   # one JSON object per line, for piping
acl <command> -o json    # a single JSON array
acl <command> -o csv     # spreadsheet friendly
acl <command> -o tsv     # tab-separated
acl <command> -o url     # just the URL column
acl <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
acl <command> --fields id,title,url
```
...
```bash
acl <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
acl <command>            # a table, because this is a terminal
acl <command> | wc -l    # JSONL, because this is a pipe
```
