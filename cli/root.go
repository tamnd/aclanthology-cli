...
type App struct {
	client *aclanthology.Client
	cfg    aclanthology.Config
...
func Root() *cobra.Command {
	app := &App{cfg: aclanthology.DefaultConfig()}

	root := &cobra.Command{
		Use:   "acl",
		Short: "Browse ACL Anthology NLP/CL research papers.",
		Long: `acl reads the ACL Anthology at aclanthology.org — a comprehensive
collection of NLP and computational linguistics research papers from ACL,
EMNLP, NAACL, EACL, and hundreds of workshops.

Paper search is powered by the Semantic Scholar public API, which requires no
API key. Volume and paper lookups scrape aclanthology.org directly. Records
are returned as table, JSON, JSONL, CSV, TSV, or URLs.

acl is an independent tool and is not affiliated with the ACL Anthology.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = aclanthology.NewClient(a.cfg)
	return nil
}
